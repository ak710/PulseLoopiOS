...
final class ReadinessDetailTests: XCTestCase {
    ...
    func testMissingKindsNamesEverySignalWithoutARow() throws {
        ...
        let row = ReadinessDaily(
            ...
            contributorsJSON: #"[{"kindRaw":"sleep","earned":30,"maxPoints":30,"value":92,"detail":"Sleep score 92"},{"kindRaw":"hrv","earned":24,"maxPoints":30,"value":47,"baseline":50,"deviation":-6,"detail":"HRV 6% below your baseline"}]"#
        )
        context.insert(row)
        try? context.save()
